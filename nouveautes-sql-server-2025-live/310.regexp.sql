...
ALTER DATABASE PachadataFormation SET COMPATIBILITY_LEVEL = 170;
GO
...
SELECT
    ...
    Telephone,
    CASE
        WHEN REGEXP_LIKE(Telephone, '^(\+33|0)[1-9]([0-9]{8})$')
            THEN 'Valid French phone number'
        WHEN REGEXP_LIKE(Telephone, '^[0-9]{2}[\s\.-][0-9]{2}[\s\.-][0-9]{2}[\s\.-][0-9]{2}[\s\.-][0-9]{2}$')
...
SELECT 
    ...
    REGEXP_INSTR(CONCAT(Prenom, ' ', Nom), '\s') AS FirstSpace,
    REGEXP_INSTR(CONCAT(Prenom, ' ', Nom), '\s', 1, 2) AS SecondSpace
FROM Contact.Contact
...
SELECT TOP 20
    c.ContactId,
    c.Prenom,
    s.value as PrenomPart,
    s.ordinal as [Order]
FROM Contact.Contact c
CROSS APPLY REGEXP_SPLIT_TO_TABLE(c.Prenom, '[\s\-]') s
WHERE c.Prenom IS NOT NULL 
  AND REGEXP_LIKE(c.Prenom, '[\s\-]');
...
FROM Contact.Contact
WHERE Telephone LIKE '01%' 
   OR Telephone LIKE '02%' 
   OR Telephone LIKE '03%';

-- REGEXP_LIKE approach (complete)
SELECT COUNT(*) as ValidationREGEXP
FROM Contact.Contact
WHERE REGEXP_LIKE(Telephone, '^(\+33|0)[1-9][0-9]{8}$');

-- SARGability

CREATE INDEX ix_Contact_Phone ON Contact.Contact (Telephone)
...
SELECT COUNT(*) as ValidationREGEXP
FROM Contact.Contact
WHERE REGEXP_LIKE(Telephone, '^[1-9][0-9]{8}$');

SELECT COUNT(*) as ValidationREGEXP
FROM Contact.Contact
WHERE REGEXP_LIKE(Telephone, '^06[0-9]{6}$');

-- row goal ?
SELECT TOP 10 Nom, Telephone
FROM Contact.Contact
WHERE REGEXP_LIKE(Telephone, '^(\+33|0)[1-9][0-9]{8}$');
GO
...
WITH QualiteData AS (
    SELECT 
        ...
        CASE WHEN REGEXP_LIKE(Telephone, '^(\+33|0)[1-9][0-9]{8}$')
             THEN 1 ELSE 0 END as ValidPhone,
       CASE WHEN REGEXP_LIKE(Prenom, '^[A-Za-zÀ-ÿ\s\-\''.]+$')
           THEN 1 ELSE 0 END as ValidFirstName,
       CASE WHEN REGEXP_LIKE(Nom, '^[A-Za-zÀ-ÿ\s\-\''.]+$')
